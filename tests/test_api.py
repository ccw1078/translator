...
class TestAPI(unittest.TestCase):
    ...
    def test_translate_api_with_word_output(self, mock_generate, mock_translate_with_vocabulary):
        """测试带Word输出的翻译API功能"""
        # 设置模拟函数返回值
    ...
    def test_translate_api_invalid_format(self):
        ...
        data = json.loads(response.data)
        self.assertFalse(data['success'])
        self.assertEqual(data['error'], "无效的输出格式，必须是 'json' 或 'word'")
    ...
    def test_translate_api_with_vocabulary(self, mock_translate_with_vocabulary):
        ...
        mock_vocabulary = [
            ...
            {"english": "deep learning", "chinese": "深度学习", "explanation": "机器学习的一个子领域"}
        ]
        mock_translate_with_vocabulary.return_value = ("这是机器学习和深度学习的示例", mock_vocabulary)
        ...
        mock_translate_with_vocabulary.assert_called_once_with(
            'This is an example of machine learning and deep learning', 
            True
        )
    
    @patch('app.api.routes.translate_with_vocabulary')
    @patch('app.api.routes.generate_word_document')
    def test_translate_api_with_vocabulary_and_word(self, mock_generate, mock_translate_with_vocabulary):
        """测试同时包含词汇表和Word输出的翻译API功能"""
        # 设置模拟函数返回值
        mock_vocabulary = [
            {"english": "test", "chinese": "测试", "explanation": "验证功能正确性的过程"}
        ]
