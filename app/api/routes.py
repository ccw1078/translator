...
from app.services.translator import translate_with_vocabulary, translate_with_vocabulary_stream
...
def build_translation_response(translation, vocabulary, include_vocabulary, output_format, text):
    """
    构建翻译响应
...
def translate():
    ...
    try:
        ...
        # 构建响应
        response = build_translation_response(translation, vocabulary, include_vocabulary, output_format, text)
...
def translate_stream():
    ...
    try:
        ...
        # 流式响应不支持Word文档格式
        if data.get('output_format') == 'word':
            return jsonify({"success": False, "error": "流式响应不支持Word文档格式"}), 400
            ...
            try:
                ...
                for chunk in translate_with_vocabulary_stream(text, include_vocabulary):
                    ...
                    if isinstance(chunk, tuple) and len(chunk) == 2:
                        ...
                        # 构建最终的词汇表JSON响应
                        response_data = {"success": True, "type": "complete", "translation": full_translation}
