...
def translate_with_vocabulary_stream(text, include_vocabulary=False):
    ...
    try:
        ...
        # 发送流式请求到DeepSeek API
        logger.info(f"发送流式翻译请求到DeepSeek API，文本长度: {len(text)} 字符, 包含词汇表: {include_vocabulary}")
        ...
        # 处理完整的响应内容以提取词汇表（如果需要）
        translation = full_content
        vocabulary_list = []
        
        if include_vocabulary:
            # 分离翻译和词汇表部分
            if '===专业词汇===' in full_content:
                parts = full_content.split('===专业词汇===')
                translation = parts[0].strip()
                vocabulary_json = parts[1].strip()
                ...
                try:
                    ...
                    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                        ...
                        try:
                            ...
                            logger.error("清理后的词汇提取结果仍然不是有效的JSON格式")
                            vocabulary_list = [{"english": "Unknown", "chinese": "未知", "explanation": "词汇提取失败"}]
                    else:
                        logger.error("无法从响应中提取JSON格式的数据")
                        vocabulary_list = [{"english": "Unknown", "chinese": "未知", "explanation": "词汇提取失败"}]
            else:
        ...
        yield (translation, vocabulary_list)
